...
const ProductSection = ({ title, products }) => {
  ...
  const handleScroll = () => {
    if (scrollRef.current) {
      ...
      // هل وصلنا للأول أو للآخر؟
      setIsAtStart(scrollLeft <= 1);
      setIsAtEnd(Math.ceil(scrollLeft + clientWidth) >= scrollWidth);

      // حساب النقطة النشطة بناءً على نسبة السكرول
  ...
  return (
    <div className="mb-16 relative">
      ...
      <div className="relative group">
        {/* سهم الشمال - ظاهر دائماً ويختفي فقط عند البداية */}
        <button 
          onClick={() => scroll('left')}
        ...
        {/* سهم اليمين - ظاهر دائماً ويختفي فقط عند النهاية */}
        <button 
        ...
        >
          {products.map((product) => (
            <div key={product.id} className="group/item cursor-pointer flex-none w-[65%] sm:w-[45%] md:w-[23%] snap-start">
              <div className="aspect-[3/4] overflow-hidden bg-[#f8f9fa] mb-4 relative rounded-sm">
                ...
                {/* زرار القلب */}
                <button className="absolute top-3 right-3 w-8 h-8 md:w-9 md:h-9 bg-white flex items-center justify-center rounded-full shadow-md opacity-0 group-hover/item:opacity-100 transition-all duration-300 hover:text-[#004b93] text-gray-400 z-10 translate-y-[-10px] group-hover/item:translate-y-0">
